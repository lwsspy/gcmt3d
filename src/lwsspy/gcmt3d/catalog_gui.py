...
from lwsspy.gcmt3d.io import read_measurements_label
from lwsspy.gcmt3d.plot_measurements import plot_measurements
...
class CatalogExplore:
    ...
    def _mpl_connect(self):
        ...
        if self.old_database and self.new_database:
            ...
            self.bmeas.on_clicked(self.onbuttonclick_measurements)
    ...
    def onbuttonclick_measurements(self, event):
        ...
        # Read measurement file
        before = read_measurements_label(
            ocmtdir, 'before')
        if not before:
            self.logger.debug('Before couldnt be found')

        after = read_measurements_label(
            ncmtdir, 'gcmt3d+_fix'
        )
        if not after:
